...
export class HttpClient {
  ...
  public get<T, R = AxiosResponse<T>>(url: string, config?: AxiosRequestConfig<T>): Promise<R> {
    return this.instance.get<T, R>(url, config);
  }

  public post<T, B = unknown, R = AxiosResponse<T>>(url: string, body: B, config?: AxiosRequestConfig<T>): Promise<R> {
  ...
  public patch<T, B = unknown, R = AxiosResponse<T>>(url: string, body: B): Promise<R> {
